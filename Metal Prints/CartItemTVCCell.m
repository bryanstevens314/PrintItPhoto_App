...
//              ];
    ...
    int ratio = image1.size.width  /image1.size.height;
    if (image1.size.width < image1.size.height) {
        NSLog(@"portrait");
        float newWidth = [self sharedAppDelegate].window.frame.size.height-20 * ratio;
        
        UIImageView *imageImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, newWidth, [self sharedAppDelegate].window.frame.size.height-20)];
    ...
    if (image1.size.width == image1.size.height) {
        NSLog(@"square");
        UIImageView *imageImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [self sharedAppDelegate].window.frame.size.width, [self sharedAppDelegate].window.frame.size.width)];
        imageImageView.center = CGPointMake([self sharedAppDelegate].window.bounds.size.width/2, [self sharedAppDelegate].window.bounds.size.height/2);
        imageImageView.image = image1;
        NSLog(@"Image:%@",image1);
        [imageController.view addSubview:imageImageView];
